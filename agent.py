...
OPENROUTER_MODEL = os.getenv("OPENROUTER_MODEL", "openrouter/gemini-2.0-flash-00")
MODEL = OPENROUTER_MODEL   # ← server.py imports this# ← loads .env before anything else
...
def build_initial_workflow_state() -> dict[str, Any]:
    ...
    packet = build_claim_intake_packet(
        claim, validation, classification, coverage, checklist, fraud_gate,
    )
    return {
